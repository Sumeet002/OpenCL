...
void openCLConvolute(cl_uchar *src_img_ , cl_uchar *out_img_ , float *conv_mat, 
	...
	status = clSetKernelArg(conv_kernel,0,sizeof(cl_mem), (void*) &dout_img_);
  	STATUSCHKMSG("in arg 2 setting");
	
	status = clSetKernelArg(conv_kernel,0,sizeof(cl_mem), (void*) &dconv_mat);
...
int main(){
	...
	img_sz = (src_img.cols/scale)*(src_img.rows/scale);
