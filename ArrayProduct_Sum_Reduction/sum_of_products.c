...
int main(){
	...
	float arrB[numElements];
	float arrC[numWorkGroups];
	...
	FILE *fp;
	fp = fopen("sumof_arrayproducts.cl","r");
	...
	err = clEnqueueWriteBuffer(command_queue, A, CL_TRUE, 0, in_arr_size, arrA, 0, NULL, NULL);
  	err = clEnqueueWriteBuffer(command_queue, B, CL_TRUE, 0, in_arr_size, arrB, 0, NULL, NULL);
	...
	kernel = clCreateKernel(program,"sumof_arrayproducts",&err );
	...
	err = clEnqueueReadBuffer(command_queue, C , CL_TRUE, 0, out_arr_size , arrC, 0, NULL, NULL);
	...
	printf("Sum of products of each elements of array is: %f\n",sum);
	
	int sum_of_squares;

	sum_of_squares = ((numElements-1)*((numElements-1)+1)*(2*(numElements-1)+1))/6;

	if(sum == sum_of_squares){
		printf("Test passed :)\n");
	}
	else{
		printf("Test failed :( value obtained is %f and sum_of_squares is %d\n",sum,sum_of_squares);
	}
