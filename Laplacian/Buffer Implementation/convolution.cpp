...
void openCLConvolute(cl_uchar *src_img_ , cl_uchar *out_img_ , int *conv_mat, 
	...
	cl_mem dconv_mat = clCreateBuffer(params.context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
						conv_mat_sz*conv_mat_sz*sizeof(cl_int),(void *)conv_mat, &status);
  	STATUSCHKMSG("i/p memory allocation ");
...
int main(){
	...
	int conv_mat[9] = {0,-1,0,-1,4,-1,0,-1,0};
